...
<?php $keywords = ''; ?>
<?php $keywords = rtrim($keywords, ', '); ?>
@section('keywords')
    {{ $keywords }}
@endsection

@section('image')
    {{ URL::to('/') }}{{ ImageHandler::getImage($video->image, 'large') }}
@endsection

@section('css')
    <link href="/assets/video/video-js.css" rel="stylesheet">
...
@section('content')
    ...
    <div id="video_bg" style="background-image:url('/images/{{ str_replace(' ', '%20', $video->image) }}')">
        <div id="video_bg_dim"
            @if($video->access != 'guest' || ($video->access != 'subscriber' && !\Auth::guest()) )
                class="darker"
            @endif
        ></div>
        <div class="container">
            @if($video->access == 'guest' || ( ($video->access == 'subscriber' || $video->access == 'registered') && !Auth::guest()) || (!Auth::guest() && (Auth::user()->role == 'demo' || Auth::user()->role == 'staff')) || (!Auth::guest() && $video->access == 'registered' && $settings->free_registration && Auth::user()->role == 'registered') )
                @if($video->type == 'embed')
                ...
                @else
                    <div id="video_container">
                        <video id="video_player" class="video-js vjs-default-skin" controls preload="auto" poster="<?= Config::get('site.uploads_url') . '/images/' . $video->image ?>" data-setup="{}" width="100%" height='500px' style="width:100%;height:500px;">
                            <source src="{{ $settings->server_link.$video->video_file_name }}" type='video/mp4'>
            ...
            @else
                <div id="subscribers_only">
                    ...
                    @if(!\Auth::guest() && $video->access == 'subscriber')
                        <form method="get" action="/users/{{\Auth::user()->username }}/upgrade_subscription">
    ...
    <div class="container video-details">
        <h3>
            ...
            <div class="favorite btn btn-default <?php if(isset($favorited->id)) echo 'active';?>" data-authenticated="<?= !Auth::guest() ?>" data-videoid="<?= $video->id ?>"><i class="fa fa-heart"></i> Favorite</div>
        </h3>
