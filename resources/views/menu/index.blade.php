	...
	<div class="admin-section-title">
		<div class="row">
			<div class="col-md-12">
				...
				<a href="javascript:;" onclick="jQuery('#add-new').modal('show');" class="btn btn-success">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="new-menu-form" accept-charset="UTF-8" action="/dashboard/menu/store" method="post">
						@csrf
				...
				<div id="nestable" class="nested-list dd with-margins">
					...
					@foreach($menu as $menu_item)
						...
						<li class="dd-item @if($menu_item->type == 'videos' || $menu_item->type == 'posts') video_post @endif" data-id="{{ $menu_item->id }}">
							...
							<div class="actions">
								<a href="{{route('dashboard.menu.edit', ['menu' => $menu_item->id]) }}" class="edit">Edit</a> 
								<form action="{{ route('dashboard.menu.destroy', ['menu' => $menu_item->id]) }}" method="post">
									@method('DELETE')
									@csrf
		...
		jQuery(document).ready(function($){
			...
			$('.dd').on('change', function(e) {
				if($('.video_post').parents('.dd-list').length > 1){
					...
					window.location = '/dashboard/menu?menu_first_level=true';
				} else {
				
	    			$('.menu-panel').addClass('reloading');
	    			$.post('/dashboard/menu/order', { order : JSON.stringify($('.dd').nestable('serialize')), _token : $('#_token').val() }, function(data){
	    				console.log(data);
