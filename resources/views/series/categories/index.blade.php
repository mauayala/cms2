...
@section('content')
	...
	<div class="admin-section-title">
		<div class="row">
			<div class="col-md-12">
				<h3><i class="entypo-archive"></i> Serie Categories</h3><a href="javascript:;" onclick="jQuery('#add-new').modal('show');" class="btn btn-success"><i class="fa fa-plus-circle"></i> Add New</a>
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">New Serie Category</h4>
				</div>
				<div class="modal-body">
					<form id="new-cat-form" accept-charset="UTF-8" action="/dashboard/series/categories/store" method="post">
						@csrf
				        ...
				        <label for="slug">URL slug (ex. series/categories/slug-name)</label>
				...
				<ol class="dd-list">
					...
					@foreach($serie_categories as $category)
						...
						<li class="dd-item" data-id="{{ $category->id }}">
							<div class="dd-handle">{{ $category->name}}<span class="slug">series/categories/{{ $category->slug }}</span></div>
							<div class="actions">
								<a href="{{route('dashboard.series.categories.edit', ['category' => $category->id]) }}" class="edit">Edit</a> 
								<form action="{{ route('dashboard.series.categories.destroy', ['category' => $category->id]) }}" method="post">
									@method('DELETE')
									@csrf
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				...
				$('.dd').on('change', function(e) {
					...
					$.post('/dashboard/series/categories/order', { order : JSON.stringify($('.dd').nestable('serialize')), _token : $('#_token').val()  }, function(data){
						console.log(data);
