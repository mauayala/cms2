		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="new-cat-form" accept-charset="UTF-8" action="/dashboard/videos/categories/store" method="post">
						@csrf
				...
				<ol class="dd-list">
					...
					@foreach($video_categories as $category)
						...
						<li class="dd-item" data-id="{{ $category->id }}">
							...
							<div class="actions">
								<a href="{{route('dashboard.videos.categories.edit', ['category' => $category->id]) }}" class="edit">Edit</a> 
								<form action="{{ route('dashboard.videos.categories.destroy', ['category' => $category->id]) }}" method="post">
									@method('DELETE')
									@csrf
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				...
				$('.dd').on('change', function(e) {
					...
					$.post('/dashboard/videos/categories/order', { order : JSON.stringify($('.dd').nestable('serialize')), _token : $('meta[name="csrf-token"]').attr('content')  }, function(data){
						console.log(data);
