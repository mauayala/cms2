...
@section('content')
    ...
    <div id="admin-container">
        ...
        <div class="admin-section-title">
            <h3>{{ $video->title }}</h3>
            <a href="{{ url('video') . '/' . $video->id }}" target="_blank" class="btn btn-info">
        ...
        <form method="POST" action="{{ route('dashboard.videos.update', ['video' => $video->id]) }}" accept-charset="UTF-8" file="1" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="panel panel-primary" data-collapsed="0"> <div class="panel-heading">
                ...
                <div class="panel-body" style="display: block;">
                    <div class="form-group">
                        <input type="text" class="form-control" name="title" id="title" placeholder="Video Title" value="{{ $video->title }}" />
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control" name="title_es" id="title_es" placeholder="Serie Title Spanish" value="{{ $video->title_es }}" />
                    </div>
                    <div class="form-group">
                        <textarea class="form-control" name="plot" id="plot" placeholder="Plot">{{ htmlspecialchars($video->plot) }}</textarea>
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control" name="actors" id="actors" placeholder="Actors" value="{{ $video->actors }}" />
                    </div>
                    <div class="col-md-4" style="padding-left: 0;">
                        <div class="form-group">
                            <input type="text" class="form-control" name="director" id="director" placeholder="Director" value="{{ $video->director }}" />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="text" class="form-control" name="released_at" id="released_at" placeholder="Release Date" value="{{ $video->released_at }}" />
                        </div>
                    </div>
                    <div class="col-md-4" style="padding-right: 0;">
                        <div class="form-group">
                            <input type="text" class="form-control" name="imdb_rating" id="imdb_rating" placeholder="IMDB Rating" value="{{ $video->imdb_rating }}" />
                        </div>
                    </div>
                    <div class="col-md-4" style="padding-left: 0;">
                        <div class="form-group">
                            <input type="text" class="form-control" name="runtime" id="runtime" placeholder="Runtime (in seconds)" value="{{ $video->runtime/60 }}" />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <select class="form-control" id="video_category_id" name="video_category_id" required="">
                                ...
                                @foreach($video_categories as $category)
                                    <option value="{{ $category->id }}" @if($video->video_category_id == $category->id)selected="selected"@endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4" style="padding-right: 0;">
                        <div class="form-group">
                            <select class="form-control" id="rating" name="rating">
                                <option value="NR" @if($video->rating == 'NR')selected="selected"@endif>NR</option>
                                <option value="G" @if($video->rating == 'G')selected="selected"@endif>G</option>
                                <option value="PG" @if($video->rating == 'PG')selected="selected"@endif>PG</option>
                                <option value="PG-13" @if($video->rating == 'PG-13')selected="selected"@endif>PG-13</option>
                                <option value="R" @if($video->rating == 'R')selected="selected"@endif>R</option>
                                <option value="NC-17" @if($video->rating == 'NC-17')selected="selected"@endif>NC-17</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="panel panel-primary" data-collapsed="0"> 
                ...
                <div class="panel-body" style="display: block;">
                    <div class="col-md-6">
                        <img src="https://s3.amazonaws.com/ctv3/{{ $video->image }}"  class="video-img" width="214" height="306" />
                    ...
                    <div class="col-md-6">
                        <img src="https://s3.amazonaws.com/ctv3/{{ $video->backdrop }}"  class="backdrop-img" width="200"/>
                    ...
                    <div class="col-md-6">
                        <img src="{{ $video->full_logo }}"  class="backdrop-img" width="200"/>
                        <p>Select the movie logo image (1280x720 px or 16:9 ratio):</p>
            ...
            <div class="panel panel-primary" data-collapsed="0"> 
                ...
                <div class="panel-body" style="display: block;">
                    ...
                    <select id="type" name="type">
                        <option value="vod" @if($video->hls_link == '') selected='selected' @endif>VOD</option>
                        <option value="hls" @if($video->hls_link != '') selected='selected' @endif>HLS</option>
                    </select>
            ...
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-body">
                    <div>
                        ...
                        <input type="checkbox" @if($video->en == 1) checked="checked" @endif name="en" value="1" id="en" />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="es" style="float:left; display:block; margin-right:10px;">Audio ES:</label>
                        <input type="checkbox" @if($video->es == 1) checked="checked" @endif name="es" value="1" id="es" />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="featured" style="float:left; display:block; margin-right:10px;">Is this movie Premiered:</label>
                        <input type="checkbox" @if($video->featured == 1) checked="checked" @endif name="featured" value="1" id="featured" />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="active" style="float:left; display:block; margin-right:10px;">Is this movie Active:</label>
                        <input type="checkbox" @if($video->active == 1) checked="checked" @endif name="active" value="1" id="active" />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="hd" style="float:left; display:block; margin-right:10px;">Is this movie HD:</label>
                        <input type="checkbox" @if($video->hd == 1) checked="checked" @endif name="hd" value="1" id="hd" />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="movies" style="float:left; display:block; margin-right:10px;">Kids Zone - Movies:</label>
                        <input type="radio" name="kids_zone" value="1" id="movies" @if($video->kids_zone == 1) checked="checked" @endif />
                    </div>
                    <div class="clear"></div>
                    <div>
                        <label for="toddlers" style="float:left; display:block; margin-right:10px;">Kids Zone - Toddlers:</label>
                        <input type="radio" name="kids_zone" value="2" id="toddlers" @if($video->kids_zone == 2) checked="checked" @endif />
                    </div>
                </div>
            </div>
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-body">
                    @if($video->user)
                        <div>
                            Created by: <strong>{{ $video->user->username }}</strong> on <strong>{{ $video->created_at }}</strong>
                        </div>
                    @endif
                    <div class="clear"></div>
                    @if(isset($video->editor->username))
                        <div>
                            Updated by: <strong>{{ $video->editor->username }}</strong> on <strong>{{ $video->updated_at }}</strong>
                        </div>
                    @endif
                </div>
            </div>
